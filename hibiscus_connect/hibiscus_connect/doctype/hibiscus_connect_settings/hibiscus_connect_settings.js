...
frappe.ui.form.on('Hibiscus Connect Settings', {
	refresh: function(frm) {
		...
		// Test MySQL Connection button (only if MySQL is enabled)
		if (frm.doc.mysql_enabled) {
			frm.add_custom_button(__('Test MySQL Connection'), function(){
				frappe.call({
					method: 'test_mysql_connection',
					doc: frm.doc,
					freeze: true,
					freeze_message: __('Testing MySQL connection...'),
				});
			});
		}

		// Refresh direct debit cache button (only if MySQL is enabled)
		if (frm.doc.mysql_enabled) {
			frm.add_custom_button(__('Refresh Direct Debit Cache'), function(){
				frappe.call({
					method: 'refresh_lastschrift_cache',
					doc: frm.doc,
					freeze: true,
